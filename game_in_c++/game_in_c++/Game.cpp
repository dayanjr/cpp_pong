...
float ball_p_x, ball_p_y, ball_dp_x = 170, ball_dp_y, ball_half_size = 1;
...
internal bool 
aabb_vs_aabb(float plx, float ply, float hslx, float hsly, float p2x, float p2y, float hs2x, float hs2y) {
    return(plx + hslx > p2x - hs2x &&
        plx - hslx < p2x + hs2x &&
        ply + hsly > p2y - hs2y &&
        ply + hsly < p2y + hs2y);
}
...
simulate_game(Input* input, float dt) {
    ...
    if (current_gamemode == GM_GAMEPLAY){
        ...
        if (!enemy_is_ai) {
            ...
            // if (ball_p_y > player_1_p+2.f) player_1_ddp += 1100;
            // if (ball_p_y < player_1_p - 2.f) player_1_ddp -= 1100;
        ...
        //simulate ball
        {
            ...
            if (player_1_score == 10) {
                ball_dp_x = 0;
                draw_text("PII LOST", -45, 30, 1, 0xff0000);
            }
            if (player_2_score == 10) {
                ball_dp_x = 0;
                draw_text("PI LOST", -45, 30, 1, 0xff0000);
            }
        }
       ...
       if (pressed(BUTTON_W) || pressed(BUTTON_S)) {
       ...
       if (pressed(BUTTON_DOWN)) {
        ...
        if (hot_button == 0) {
            ...
            draw_text("MULTIPLAYER", 10, -10, 1, 0xaaaaaa);
        }
        else {
        ...
        draw_text("CPP PONG", -45, 30, 2, 0xffffff);
    }
}